...
int log2(int x){
	int ii;
	int count;
	for( ii = N_ELEMS, count = 0; ii > 1 ; ii /= 2, count++);
	return count;
}
...
void transform_points_AOS(INOUT POINT_ELEMENT *p_AOS, IN int n_points, IN int m)
{
	...
	printf("*** GPU 1 E:%d, N:%d, B:(%d,%d), M:global   : GPU Time taken = %.3f ms\n", 
		ELEM_PER_POINT, log2(N_ELEMS), BLOCK_WIDTH, BLOCK_HEIGHT, elapsedTime);
...
void transform_points_SOA(INOUT POINTS_SOA p_SOA, IN int n_points, IN int m)
{
	...
	float elapsedTime;
	cudaEventElapsedTime(&elapsedTime, start, stop);
	printf("*** GPU 2 E:%d, N:%d, B:(%d,%d), M:global   : GPU Time taken = %.3f ms\n",
		ELEM_PER_POINT, log2(N_ELEMS), BLOCK_WIDTH, BLOCK_HEIGHT, elapsedTime);
	cudaEventDestroy(start);
	cudaEventDestroy(stop);
...
{
	float compute_time, device_time;
	cudaEvent_t start, stop;
	cudaEventCreate(&start);
	cudaEventCreate(&stop);
	...
	float elapsedTime;
	cudaEventElapsedTime(&elapsedTime, start, stop);
	printf("*** GPU 1 E:%d, N:%d, B:(%d,%d), M:constant : GPU Time taken = %.3f ms\n",
		ELEM_PER_POINT, log2(N_ELEMS), BLOCK_WIDTH, BLOCK_HEIGHT, elapsedTime);
	cudaEventDestroy(start);
	cudaEventDestroy(stop);
}
...
void transform_points_SOA_with_constant(INOUT POINTS_SOA p_SOA, IN int n_points, IN int m)
{
	...
	float elapsedTime;
	cudaEventElapsedTime(&elapsedTime, start, stop);
	printf("*** GPU 2 E:%d, N:%d, B:(%d,%d), M:constant : GPU Time taken = %.3f ms\n",
		ELEM_PER_POINT, log2(N_ELEMS), BLOCK_WIDTH, BLOCK_HEIGHT, elapsedTime);
	cudaEventDestroy(start);
	cudaEventDestroy(stop);
