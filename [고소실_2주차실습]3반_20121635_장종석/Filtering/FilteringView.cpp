...
void CFilteringView::AverageFiltering(int w0)
{
	...
	for (r=0; r<m_height; r++) { // 세로
		for (c=0; c<m_width; c++) { // 가로
			mean = 0;
...
void CFilteringView::MedianFiltering(int w0)
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			//
			// Problem 2-a1,a2,a3: imageBits 에 median Average Filtering 처리를 하여 imageBitsFiltered 에 할당한다
