...
void my_cvSmooth(const IplImage* src, IplImage* dst, int kernel_size, double sigma1){
	...
	if (kernel_size == 0)
		kernel_size = cvRound(sigma1*6) +1 | 1;		//	Yoon-Designed. (Result: 95 / 0)
													//	reference : https://github.com/opencv/opencv/blob/master/modules/imgproc/src/smooth.cpp
	//printf("kernel_size : %lf, %d, %d\n", sigma1, kernel_size, channels);
		...
		for (c=0; c<m_width; c++){
			...
			for( l=-w; l<=w; l++){
				for( k=-w; k<=w; k++){
					...
					if( ( 0 <= r+l ) && ( r+l < m_height ) && ( 0 <= c+k ) && ( c+k < m_width) ){
						gaussian += cvGet2D(src,r+l,c+k).val[0] * _2d_Gaussian_Kernel.at<double>(w+l,w+k);
					}
				}
			}
